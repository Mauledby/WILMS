...
$("#finish-btn").click(function() {
	var err = false;
	$("#fieldset5").find(".mentor-days").each(function(index) {
		var days_selected = 0;
	...
	$("#fieldset5").find("input").each(function(index) {
		if ($(this).val().trim() == "") {
			err_mesg(this, "Please enter " + $(this).parent().children().first().text());
			if (!err) {
				err = true;
	...
	$("#fieldset5").find(".mentor-to").each(function(index) {
		if ($(this).val() <= $($("#fieldset5").find(".mentor-from").get(index)).val()) {
	...
	if (!err) {
		...
		var mentor_from = to_str_array(".mentor-from");
		var mentor_to = to_str_array(".mentor-to");
		$.ajax({
			url: 'ajax/men_reg',
			data: {
				...
				'mentor_mondays': mentor_mondays,
				'mentor_tuesdays': mentor_tuesdays,
				'mentor_wednesdays': mentor_wednesdays,
				'mentor_thursdays': mentor_thursdays,
				'mentor_fridays': mentor_fridays,
				'mentor_saturdays': mentor_saturdays,
				'mentor_sundays': mentor_sundays,
				'mentor_from': mentor_from,
				'mentor_to': mentor_to
			},
			dataType: 'json',
