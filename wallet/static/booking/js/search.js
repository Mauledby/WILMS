function autocomplete(inp, arr, txt, ids, next, self) {
	...
	att.setAttribute("id", self);
	att.setAttribute("class", "d-flex justify-content-between");
	attRem = document.createElement("BUTTON");
	attRem.innerHTML = "Remove";
	attRem.setAttribute("class", "btn btn-danger");
	attRem.addEventListener("click", function(e) {
		...
		var table = document.getElementById("val_attendees").getElementsByTagName('tbody')
	...
	attName.innerHTML = self;
	att.appendChild(attName);
	att.appendChild(attRem);
	inp.value = "";
	added.push(self);
	var table = document.getElementById("val_attendees").getElementsByTagName('tbody')[0];
	var row = table.insertRow();
	row.setAttribute("id", self);
	var cell = row.insertCell(); 
	cell.setAttribute("id", self);
	cell.append(att);

	inp.addEventListener("input", function(e) {
			var autocompleteList, name, i, val = this.value;
			//console.log(added)
			...
			for (i = 0; i < arr.length; i++) {
				...
				if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase() && !added.includes(arr[i])) {
					...
					name.innerHTML = "<strong>" + arr[i].substr(0, val.length) + "</strong>";
					name.innerHTML += arr[i].substr(val.length);
					/*insert a input field that will hold the current array item's value:*/
					name.innerHTML += "<input type='hidden' value='" + arr[i] + "'>";
					/*execute a function when someone clicks on the item value (DIV element):*/
							name.addEventListener("click", function(e) {
								...
								attRem = document.createElement("BUTTON");
								attRem.innerHTML = "Remove";
								attRem.setAttribute("class", "btn btn-danger float-right");
								attRem.addEventListener("click", function(e) {
									e.preventDefault();
									var currTxt = txt.value.split(',');
									var currIds = ids.value.split(',');
								...
								inp.value = "";
								//list.appendChild(att); //appends to bottom list
								added.push(this.getElementsByTagName("input")[0].value);
								
								var table = document.getElementById("val_attendees").getElementsByTagName('tbody')[0];
								var row = table.insertRow();
								row.setAttribute("id", this.getElementsByTagName("input")[0].value);
								var cell = row.insertCell(); 
								cell.setAttribute("id", self);
								cell.append(att);
					...
					autocompleteList.appendChild(name); //shows list of names in autocomplete
				}
			}
	});
