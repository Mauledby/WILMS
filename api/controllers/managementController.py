...
class ManagementController():
    ...
    def getAllUserBookings(request, user_id):
        today = datetime.now()       
        my_bookings = Booking.objects.filter(user=user_id).order_by('-date','-startTime')
        serializer = BookingSerializer(my_bookings, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    @api_view(['GET'])
    def getUpcomingUserBookings(request, user_id):
        today = datetime.now()       
        upcoming_bookings = Booking.objects.filter(date__gte=today, user=user_id).order_by('-date','-startTime')
    ...
    def getHistoryUserBookings(request, user_id):
        today = datetime.now()
        history_bookings = Booking.objects.filter(date__lt=today, user=user_id).order_by('-date','-startTime')
